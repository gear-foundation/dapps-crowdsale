...
test: build
	@if [ ! -f "./target/fungible_token.wasm" ]; then wget "https://github.com/gear-dapps/fungible-token/releases/download/build/fungible_token.wasm" -O "./target/fungible_token.wasm"; fi
	@echo ──────────── Run tests ────────────────────────
